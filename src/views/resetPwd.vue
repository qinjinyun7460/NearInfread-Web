<!-- <template>
  <el-card>
    <el-form ref="pwdRef" :model="form" :rules="rules" label-width="auto">
      <el-form-item v-if="isAdmin" label="目标用户ID" prop="target_user_id">
        <el-input v-model="form.target_user_id" placeholder="请输入要重置密码的用户ID" />
      </el-form-item>

      <el-form-item v-if="!isAdmin" label="旧密码" prop="old_password">
...
<template>
  <el-card>
    <el-form ref="pwdRef" :model="form" :rules="rules" label-width="auto">
      <!-- <div v-if="form.target_user_name" class="user-info">
        <el-tag type="info">当前重置用户：{{ form.target_user_name }} (ID: {{ form.target_user_id }})</el-tag>
      </div> -->
